...
void Robot::place(const RobotLoc& position, const Board& board)
{
    if (board.isCoordinateWithinBounds(position.getCoordinate())) {
        _position.reset(new RobotLoc(position));
    }
}
...
void Robot::move(const Board& board)
{
    ...
    auto moveCoordinate = Location(x, y);

    if (board.isCoordinateWithinBounds(moveCoordinate)) {
        _position->setCoordinate(Location(x, y));
    }
}
...
std::string Robot::report() const
{
    ...
    std::stringstream ss;
    ss << _position->getCoordinate().toString() << "," << FacingToStr(_position->getDirection());
